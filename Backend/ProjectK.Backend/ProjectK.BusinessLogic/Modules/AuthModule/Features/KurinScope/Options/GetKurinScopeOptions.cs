...
/// </summary>
public sealed record GetKurinScopeOptions(Guid UserKey)
...
public sealed class GetKurinScopeOptionsHandler
    : IRequestHandler<GetKurinScopeOptions, ServiceResult<IReadOnlyCollection<KurinScopeOption>>>
{
    private readonly IMembershipDirectory _memberships;

    public GetKurinScopeOptionsHandler(IMembershipDirectory memberships)
    {
        _memberships = memberships;
    }

    public async Task<ServiceResult<IReadOnlyCollection<KurinScopeOption>>> Handle(
        GetKurinScopeOptions request,
